...
# ===================================================================
# Framework convention entry points (predicate supplied by the caller)
# ===================================================================


class TestConventionEntry:
    """The engine stays framework-agnostic: callers pass a predicate."""

    def test_convention_entries_are_not_orphaned(self, tmp_path):
        page = _write_file(tmp_path / "app" / "page.tsx", lines=30)
        handler = _write_file(tmp_path / "server" / "api" / "users.get.ts", lines=20)
        orphan = _write_file(tmp_path / "src" / "orphan.ts", lines=25)
        graph = {
            str(page): _graph_entry(importer_count=0),
            str(handler): _graph_entry(importer_count=0),
            str(orphan): _graph_entry(importer_count=0),
        }
        seen: list[str] = []

        def is_entry(rel_path: str) -> bool:
            seen.append(rel_path)
            return rel_path.startswith(("app/", "server/api/"))
        ...
        with patch(
            ...
            entries, total = detect_orphaned_files(
                ...
                graph,
                [".ts", ".tsx"],
                options=OrphanedDetectionOptions(convention_entry=is_entry),
            )

        assert total == 3
        assert [e["file"] for e in entries] == [str(orphan)]
        assert sorted(seen) == ["app/page.tsx", "server/api/users.get.ts", "src/orphan.ts"]

    def test_no_predicate_reports_convention_files(self, tmp_path):
        page = _write_file(tmp_path / "app" / "page.tsx", lines=30)
